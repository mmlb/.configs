...
export PATH=$HOME/bin:$HOME/go/bin:$HOME/.cargo/bin:$HOME/.local/bin:$HOME/.zplug/bin:$PATH:$(realpath $(dirname $($HOME/bin/realwhich git))/../share/git/contrib/diff-highlight)
...
if [[ ${RUNNING_IN_XWAYLAND:-} == true ]]; then
  unset GDK_BACKEND
  unset I3SOCK
  unset RUNNING_IN_XWAYLAND
  unset SWAYSOCK
